...
from tkinter import filedialog, messagebox, Tk, Label, Checkbutton, Entry, simpledialog, Text, Scrollbar, END, DISABLED, NORMAL
...
def set_window_icon(window):
    ...
    if icon_path:
        try:
            ...
            logging.info(f"Icon set successfully for {window.title()} at {icon_path}")
        except Exception as e:
            error_msg = f"Failed to load icon at {icon_path} for {window.title()}: {e}"
            logging.error(error_msg)
            print(f"{Fore.YELLOW}⚠ {error_msg}{Style.RESET_ALL}")
    else:
        error_msg = f"No icon path available for {window.title()}"
        logging.error(error_msg)
...
def countdown_window(config):
    root = ttk.Window(themename="darkly")
    root.title("Injection Countdown")
    ...
    label = ttk.Label(frame, text="Preparing to inject...", font=("Segoe UI", 14, "bold"), anchor="center")
    label.pack(pady=5)
    
    time_label = ttk.Label(frame, text=f"Inject in: {config['inject_wait_time']}s", font=("Segoe UI", 12), anchor="center")
    time_label.pack(pady=10)
    
    progress = ttk.Progressbar(frame, maximum=config["inject_wait_time"], value=config["inject_wait_time"], style="success.TProgressbar")
    progress.pack(fill=X, padx=10)
    
    def update_countdown(remaining):
        if remaining >= 0:
            time_label.config(text=f"Inject in: {remaining}s")
            progress["value"] = remaining
            root.after(1000, update_countdown, remaining - 1)
        else:
            root.destroy()
    
    update_countdown(config["inject_wait_time"])
    root.mainloop()
...
def get_inject_wait_time(config):
    root = Tk()
    root.title("Inject Wait Time Prompt")
    set_window_icon(root)
    root.update()
    root.withdraw()
            ...
            if not config["dll_path"] or not os.path.exists(config["dll_path"]) or \
               (not config["use_steam"] and (not config["repo_path"] or not os.path.exists(config["repo_path"]))):
                ...
                if not config["dll_path"] or (not config["use_steam"] and not config["repo_path"]):
                    response = ttk.dialogs.dialogs.Messagebox.yesno("Configuration Required", "DLL path or Game path (if not using Steam) is still missing. Retry configuration?")
                    if not response:
                        if not NO_CONSOLE_MODE:
